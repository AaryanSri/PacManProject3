...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[GHOST0] = variableDomainsDict[PAC]
    variableDomainsDict[GHOST1] = variableDomainsDict[GHOST0]
    dis_obs0 = {
    manhattanDistance(pacman_pos, ghost_pos) + MAX_NOISE
    for pacman_pos, ghost_pos in product(variableDomainsDict[PAC], variableDomainsDict[GHOST0])
    }
    dis_obs0.update(range(MAX_NOISE))
    variableDomainsDict[OBS0] = dis_obs0

    dis_obs1 = {
        manhattanDistance(pacman_pos, ghost_pos) + MAX_NOISE
        for pacman_pos, ghost_pos in product(variableDomainsDict[PAC], variableDomainsDict[GHOST1])
    }
    dis_obs1.update(range(MAX_NOISE)) 
    variableDomainsDict[OBS1] = dis_obs1   
            ...
            for factor in F:
                ...
                if variable in factor.unconditionedVariables():
                    if len(factor.unconditionedVariables()) > 1:
                        newF.append(eliminate(factor, variable))
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        "*** YOUR CODE HERE ***"
        # get the manhattan distance between two objs
        real = manhattanDistance(pacmanPosition, ghostPosition)
        ...
        if noisyDistance is None:
            ...
            if ghostPosition != jailPosition:
                noisy = busters.getObservationProbability(noisyDistance, real)
                return noisy
            else:
                return 0.0
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        agent_pos = gameState.getPacmanPosition()
        jail = self.getJailPosition()
        for ghost_pos in self.allPositions:
            probability = self.getObservationProb(observation, agent_pos, ghost_pos, jail)
            self.beliefs[ghost_pos] *= probability
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for curr in self.allPositions:
            new_dist = self.getPositionDistribution(gameState, curr)
            for item in new_dist:
                temp[item] += new_dist[item] * self.beliefs[curr]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for _ in range(self.numParticles):
            curr_pos = self.legalPositions
            curr = curr_pos[counter]
            self.particles.append(curr)
            counter = (counter + 1) % len(curr_pos)
        "*** END YOUR CODE HERE ***"
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        ghostPos = DiscreteDistribution()

        total = 0
        
        for val in self.particles:
            ghostPos[val] = ghostPos[val] + 1
            total = total + 1
        ghostPos.normalize()
        return ghostPos
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        agent_position = gameState.getPacmanPosition()

        jail = self.getJailPosition()

        ghost_pos_prediction = DiscreteDistribution()

        for ghost_pos in self.particles:
            ghost_pos_prediction[ghost_pos] += self.getObservationProb(observation, agent_position, ghost_pos, jail)

        if ghost_pos_prediction.total() == 0:
            self.initializeUniformly(gameState)
        else:
            ghost_pos_prediction.normalize()
            self.particles = [ghost_pos_prediction.sample() for _ in range(self.numParticles)]
        "*** END YOUR CODE HERE ***"
