...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for dist in livingGhostPositionDistributions:
            for position, value in dist.items():
                if max_probability < value:
                    max_probability = value
                    max_probability = position
        successorPos = [Actions.getSuccessor(pacmanPosition, action) for action in legal]
        distances = [self.distancer.getDistance(max_position, pos) for pos in successorPos]
